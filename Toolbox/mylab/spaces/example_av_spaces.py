...
class ExampleAVSpaces(ASTSpaces):
    def __init__(self,
        ...
        self.c_n_vx_low = n_vx_low
        self.c_n_vx_high = n_vx_high
        self.c_n_vy_low = n_vy_low
        self.c_n_vy_high = n_vy_high
        self.c_n_x_low = n_x_low
        self.c_n_x_high = n_x_high
        self.c_n_y_low = n_y_low
        self.c_n_y_high = n_y_high
    ...
    def action_space(self):
        ...
        """
        low = np.array([self.c_x_accel_low, self.c_y_accel_low, self.c_n_vx_low, self.c_n_vy_low, self.c_n_x_low, self.c_n_y_low])
        high = np.array([self.c_x_accel_high, self.c_y_accel_high, self.c_n_vx_high, self.c_n_vy_high, self.c_n_x_high, self.c_n_y_high])

        for i in range(1, self.c_num_peds):
            low = np.hstack((low, np.array([self.c_x_accel_low, self.c_y_accel_low, self.c_n_vx_low, self.c_n_vy_low, self.c_n_x_low, self.c_n_y_low])))
            high = np.hstack((high, np.array([self.c_x_accel_high, self.c_y_accel_high, self.c_n_vx_high, self.c_n_vy_high, self.c_n_x_high, self.c_n_y_high])))
