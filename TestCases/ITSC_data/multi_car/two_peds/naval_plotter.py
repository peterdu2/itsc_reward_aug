...
for i in range(10,11):
    ...
    ped2_x = np.array(ped2_x) + 2.5 
    
    for frame in range(len(car_x)-1, len(car_x)):

        car1_image_path = get_sample_data('/home/peter/Research/AST_RevC/TestCases/ITSC_data/multi_car/two_peds/truck.png')
        ...
        currentAxis.add_patch(Rectangle((-50, 0.7), 100, 50, facecolor='xkcd:khaki', fill=True, alpha=0.8))
        currentAxis.add_patch(Rectangle((-50, -20), 100, 19.3, facecolor='xkcd:khaki', fill=True, alpha=0.8))
        ...
        imscatter(car1_img_x, car1_img_y, car1_image_path, zoom=0.55, ax=ax)
        #imscatter(car2_img_x, car2_img_y, car2_image_path, zoom=0.45, ax=ax)
        imscatter(peds_x, peds_y, ped_image_path, zoom=0.58, ax=ax)
        ...
        plt.plot(car_x[:frame+1], car_y[:frame+1], color='xkcd:blue', linewidth=2.5)
        #plt.plot(car2_x[:frame+1], car2_y[:frame+1], color='xkcd:red')
        #plt.plot(ped1_x[:frame+1], ped1_y[:frame+1], color='xkcd:purple')
        


        #Plot other pedestrian paths (ped2)
        other_path_idx = [0,5,6,11,12,13,15,16]
        for j in other_path_idx:
            ld = pickle.load(open('t1_w_discount/pickles/data'+str(j)+'.pkl', "rb" ))
            x = ld[8]
            y = ld[9]
            x = np.array(x) + 2.5
            plt.plot(x, y, color='xkcd:black', alpha=0.4)


        plt.plot(ped2_x[:frame+1], ped2_y[:frame+1], color='xkcd:red', linewidth=2.5)
        ...
        filepath = 'naval/T'+str(i)+'/'+str(frame)+'.jpeg'
