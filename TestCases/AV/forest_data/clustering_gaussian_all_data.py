...
models = [GaussianMixture(n, covariance_type='full', random_state=0).fit(np.array(cluster_data))
          for n in n_components]

plt.plot(n_components, [m.bic(np.array(cluster_data)) for m in models], label='BIC')
plt.plot(n_components, [m.aic(np.array(cluster_data)) for m in models], label='AIC')
plt.legend(loc='best')
plt.xlabel('n_components');

gmm = GaussianMixture(n_components=5).fit(cluster_data)
labels = gmm.predict(cluster_data)
plt.scatter(*zip(*cluster_data), c=labels, s=40, cmap='viridis');
plt.show()

filepath = 'GMM_all_data.pkl'
with open(filepath, 'wb+') as f:  
    pickle.dump([cluster_data, param_list, tree_id_list, gmm], f)
...
gmm = (pickle.load(open('GMM_all_data.pkl', 'rb')))[3]
cluster_failure_centres = (pickle.load(open('GMM_all_data.pkl', 'rb')))[0]
param_list = (pickle.load(open('GMM_all_data.pkl', 'rb')))[1]
tree_id_list = (pickle.load(open('GMM_all_data.pkl', 'rb')))[2]
