...
for i in range(num_trees):
    print(i)
    ax_min = random.uniform(-1,0)
    ax_max = -ax_min#random.uniform(0,1)
    ay_min = random.uniform(-1,0)
    ay_max = -ay_min#random.uniform(0,1)
    nvx_min = random.uniform(0,1)
    nvx_max = random.uniform(nvx_min,1)
    nvy_min = random.uniform(0,1)
    nvy_max = random.uniform(nvy_min,1)
    nx_min = random.uniform(0,1)
    nx_max = random.uniform(nx_min,1)
    ny_min = random.uniform(0,1)
    ny_max = random.uniform(ny_min,1)

    #AS_params are [[ax_min, ax_max], [ay_min, ay_max], [nvx_min, nvx_max],...,[ny_min,ny_max]]
    AS_params = [[ax_min,ax_max],[ay_min,ay_max],[nvx_min,nvx_max],[nvy_min,nvy_max],[nx_min,nx_max],[ny_min,ny_max]]
    print(AS_params)
    #Object creation for MCTS forest
    reward_function = multi_car_reward(num_peds=2)
    spaces = ExampleAVSpaces(num_peds=3,    #Set to 3, last action used by car2
    ...
    filepath = 'forest_data/F3/'+str(i)+'.pkl'
    if not os.path.exists(os.path.dirname(filepath)):
        try:
            os.makedirs(os.path.dirname(filepath))
        except OSError as exc: # Guard against race condition
